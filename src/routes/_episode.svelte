...
<div
  ...
  class="mx-6 my-4 p-2 rounded-lg bg-gray-900">
  <div
    class="bg-gray-700 h-20 relative rounded-lg mb-2 overflow-hidden cursor-pointer"
    bind:clientWidth={w}
    bind:clientHeight={h}
    bind:this={scrubber}
    ...
    {#if $others && $others[episode.meta.audio]}
      <div>
        <div
          class="absolute top-0 h-20 left-0 bg-gray-800"
  ...
  <h3 class="text-white font-thin text-2xl mb-1">{episode.title}</h3>

  <div class="text-lg font-thin text-white whitespace-pre-wrap">
    {episode.description}
  </div>
</div>
